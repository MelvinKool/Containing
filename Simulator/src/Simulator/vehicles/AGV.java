...
//    
    ...
    public void setPath(List<Vector3f> wayPoints, float durations) {
        if(wayPoints == null || durations == 0 || (motionEvent != null && motionEvent.getPlayState() == PlayState.Playing))
            return;
        //set motionpath
...
//           motionPath.addWayPoint(temp);
        this.motionPath.setPathSplineType(Spline.SplineType.Linear);
        if(wayPoints.size() > 0)
            addNewWayPointToMPath(this.wayPoints.get(0).clone());
    }
...
//    }
    ...
    public void onWayPointReach(MotionEvent motionControl, int wayPointIndex) {
        ...
        if(wayPoints.size() > 0)
        {
            ...
            Vector3f newPoint = wayPoints.get(0).clone();
            addNewWayPointToMPath(newPoint);
